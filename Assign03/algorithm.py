...
def CEMv2(test_function, dimensions, eps, bounds, popsize, num_elite, sigma_init, extra_std, seed_number, max_evals):
    np.random.seed(seed_number)
    bound_lower, bound_upper = np.asarray(bounds).T
    sigma = sigma_init * np.eye(dimensions)
    ...
    results = []
    results.append((mu.copy(), test_function(mu), n_evals))
    ...
    # for i in range(10000):
        ...
        # if test_function(mu) < eps: 
        #     break
        
    all_mu.append(mu)
    best_results = mu.copy()
    best_fitness = test_function(mu)
    return all_pops, all_sigma, results , generation_count
